...
export const addNewContact=functions.https.onCall(async (request)=>{
  const userID=request.body.userID;
  const contact=request.body.contactData;
  const contactId=id.v1();
  contact["id"]=contactId;
  const status=await admin.firestore().doc(`Users/${userID}/contacts/${contactId}`).set(contact).then(
      ()=>{
        return true;
      }
  ).catch((error)=>{
...
export const updateContact=functions.https.onCall(async (request)=>{
  ...
  const value=await admin.firestore().doc(`Users/${userID}/contacts/${contactId}`).update(updateValues).then(
      ()=>{
        return true;
      }
  ).catch(()=>{
