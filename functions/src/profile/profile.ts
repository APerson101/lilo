...
export const updateField=functions.https.onCall(async(request)=>{
    ...
    let value=await admin.firestore().doc(`Users/${userID}`).update(update).then((result)=>{
...
export const deleteAccount=functions.https.onCall(async(request)=>{
    ...
    //delete wallet first
    let url=`https://sandboxapi.rapyd.net/v1/user/${walletID}`;
  ...
  if(response["status"]["status"]=="SUCCESS")
  {
      ...
      let value=await admin.firestore().doc(`Users/${userID}`).delete().then((result)=>{
        return true;
    }).catch((error)=>{
        return false;
    });
    if(value)
    {
       ...
       let deleteStatus= await admin.auth().deleteUser(userID).then(()=>{
            return true;
