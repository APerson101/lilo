...
async  approvePayout(response:any) {
  ...
  const amount=response["amount"]
   const url:string=`https://sandboxapi.rapyd.net/v1/payouts/complete/${payout}/${amount}`
   var request={"url":url, headers: header.getHeader(null, url, 'post')}
