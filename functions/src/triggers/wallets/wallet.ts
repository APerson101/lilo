...
export const verifyEntry=functions.https.onCall(async(request)=>{
 ...
 var response=await  verifyCredentials(email, phone_number);
 if(response=='success')
 return true;
 return false;
});
...
console.log('family id: ',family_ID)
 let status= family.forEach(async(familymember:any) => {
    var wallet=await _createWallet(familymember);//
console.log('the created wallet is ',wallet)

     var walletId;var user_type=familymember["user_type"]
...
export const addFamilyMember=functions.https.onCall(async (request)=>{
  ...
  var walletId;
  var wallet=await _createWallet(userDetails);
  if(wallet["status"]["status"]=="SUCCESS")
  {
   walletId=wallet["data"]["id"];
console.log('wallet created succesfuly: id is ',walletId)
...
function _authenticateUser(email:string, password:string, phone:string, name:string)
{
      ...
      var status=  admin.auth().createUser({email:email, password:"000000",phoneNumber:phone, displayName:name}).then
        ((userRecord)=>{
...
export const addWithdraw=functions.https.onCall(async(request)=>{
  var _type=request["params"];
  var fullpath=baseURL+`account/${_type}`
