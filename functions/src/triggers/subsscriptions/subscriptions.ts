...
export const createSubscription = functions.https.onCall(async(request)=>{
   ...
   {
        //proceed
         var product_ID:string = await _createsubProduct(request["body"])
        var plan_ID:string = await _createSubPlan(product_ID, request["body"],request["body"]["payout_fields"])
...
export const createsubscriptionGroup=functions.https.onCall(async (request)=>{
  ...
  let newgroup=request["body"]["newGroup"]
...
export const removesubscriptionGroup=functions.https.onCall(async (request)=>{
  let userID=request["body"]["userID"]
  let newgroup=request["body"]["oldGroup"]
...
var body={"cancel_at_period_end":true}
